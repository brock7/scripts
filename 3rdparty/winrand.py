#!/usr/bin/python
#Scans random ips for netbios open and attempts to mount.
#Usage: ./winrand.py <how many you want to scan?>
...
def getips():
    ...
    nmapoutput = StringIO.StringIO(commands.getstatusoutput('nmap -P0 -iR 1 -p 137,138,139,445')[1]).readlines()
    
    for tmp in nmapoutput:
	ipaddr = re.findall("\d*\.\d*\.\d*\.\d*", tmp)
	if ipaddr:
	    print "Testing:",ipaddr[0]
...
if args == []:
    print 'Usage: winrand.py <how many to scan>'
    print 'ex: ./winrand.py 1000'
else:    
